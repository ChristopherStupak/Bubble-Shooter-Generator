class BubbleShooterLevelGenerator {
    ...
    generateLevel(options = {}) {
        const config = {
            ...
            birdCount: options.birdCount || 0,
            lockedChance: options.lockedChance || 0,
            bombChance: options.bombChance || 0,
            transparentChance: options.transparentChance || 0,
            stoneChance: options.stoneChance || 0
        };
    ...
    createLevel(config) {
        ...
        // Step 3: Ensure level is solvable
    ...
    placeBirds(grid, config) {
        if (config.birdCount === 0) return;
        
        let birdsPlaced = 0;
        const maxAttempts = 200;
        
        for (let attempt = 0; attempt < maxAttempts && birdsPlaced < config.birdCount; attempt++) {
            // Pick a random position (avoid first and last rows)
            const row = Math.floor(Math.random() * Math.max(1, grid.length - 2)) + 1;
    ...
    // Mirror special bubble to maintain symmetry
